...
void generarCSV(const std::string& inputFilePath, const std::string& outputFilePath, int numHilos) {
    ...
    // Cada hilo almacena su información en memoria antes de escribir
    std::vector<std::vector<std::string>> buffers(numHilos);
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 1; i <= hasta; i++) {
            ...
            // Guardar en el buffer local del hilo
            localBuffer.push_back(ss.str());
        }

        // Guardar los datos del buffer local en el buffer global
        buffers[id] = std::move(localBuffer);
    } // Fin de la región paralela

    // Sincronización: escribir en el archivo de forma ordenada
    for (const auto& buffer : buffers) {
        for (const auto& linea : buffer) {
            Utils::escribirLineaCSV(outputFilePath, linea);
        }
    }
}
